...
def __apply_demorgan(f: Formula) -> Formula:
    if isinstance(f, And):
        return And(list(map(__apply_demorgan, f.input_list)))
    elif isinstance(f, Or):
        return Or(list(map(__apply_demorgan, f.input_list)))
    elif isinstance(f, Not):
        clause = cast(Formula, f.c)
        if isinstance(clause, And):
            return __apply_demorgan(Or(list(map(lambda c: Not(c), clause.input_list))))
        elif isinstance(clause, Or):
            return __apply_demorgan(And(list(map(lambda c: Not(c), clause.input_list))))
