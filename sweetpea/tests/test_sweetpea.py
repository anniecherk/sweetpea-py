...
def test_desugar_derivation():
    ...
    assert desugar_derivation(Derivation(4, [[0, 2], [1, 3]]), blk) == to_cnf(And([
    ...
    assert desugar_derivation(Derivation(5, [[0, 3], [1, 2]]), blk) == to_cnf(And([
...
def test_desugar_full_crossing():
    assert desugar_full_crossing(25, blk) == (41, to_cnf(And([
